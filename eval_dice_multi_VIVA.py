...
tf.app.flags.DEFINE_string('checkpoint_path', '../model/hand_resnet_v1_50_dice_multi_rbox_R01/', '')
...
def detect(im_fn, ratio_h, ratio_w, score_map, geo_map, timer, score_map_thresh=0.8, box_thresh=0.1, nms_thres=0.2):
    '''
    ...
    with tf.get_default_graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            if CKPT_PATH is not None:
                ...
                # ckpt_state = tf.train.latest_checkpoint(FLAGS.checkpoint_path)
            ...
            with open(FLAGS.output_dir+'VIVA-dice-multi-R01-'+global_step+'.txt', 'w') as f:
                ii = 0
                for im_fn in im_fn_list:
                    ...
                    #updated
                    boxes, timer = detect(im_fn, ratio_h, ratio_w, score_map=score[3], geo_map=geometry[3], timer=timer)
