...
tf.app.flags.DEFINE_string('output_dir', '../result/VIVA-test-result/pos-for-time/', '')#'/tmp/ch4_test_images/images/'
...
def main(argv=None):
    ...
    with tf.get_default_graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            checkpoints = [FLAGS.checkpoint_path+'model.ckpt-10006.index']#glob.glob(FLAGS.checkpoint_path+'model.ckpt-*index')
            checkpoints.sort()
            for checkpoint in checkpoints:
                ...
                for th in np.arange(0.15, 0.7, 0.05):
                    result_file = FLAGS.output_dir+'for-images-VIVA-aug-dice-multi-'+str(th)+'-'+global_step+'.txt'
                    with open(result_file, 'w') as f:
                        ...
                        for im_fn in im_fn_list:
                            ...
                            #updated
                            boxes, timer = detect(im_fn, im, ratio_h, ratio_w, score_map=score[3], geo_map=geometry[3], box_thresh=th, timer=timer)
