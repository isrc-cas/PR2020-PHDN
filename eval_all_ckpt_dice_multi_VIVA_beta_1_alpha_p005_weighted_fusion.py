...
tf.app.flags.DEFINE_string('test_data_path', '../data/VIVA/test/', '')
tf.app.flags.DEFINE_string('gpu_list', '5', '')
tf.app.flags.DEFINE_string('checkpoint_path', '../model/hand_resnet_v1_50_dice_multi_VIVA_aug_rbox_beta_1_alpha_p005_weighted_fusion/', '')
tf.app.flags.DEFINE_string('output_dir', '../result/VIVA-test-result/pos/', '')#'/tmp/ch4_test_images/images/'
tf.app.flags.DEFINE_string('heatmap_output_dir', '../result/VIVA-test-result/heatmaps_resenet_dice_multi/', '')
tf.app.flags.DEFINE_bool('no_write_images', True, 'do not write images')

import resnet_v1_model_dice_multi_beta_1_alpha_p005_weighted_fusion as model
from VIVA_R01 import restore_rectangle

FLAGS = tf.app.flags.FLAGS
CKPT_PATH = None#'../model/hand_resnet_v1_50_dice_multi_merge_rbox/model.ckpt-70036'#
heatmaps = []
BOX_THRESH = 0.1
...
def resize_image(im, max_side_len=2400):
    ...
    # h, w = im.shape[:2]
    # resize_h, resize_w = int(h*1.5), int(w*1.5)
    # limit the max side
    ...
    with tf.get_default_graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            for checkpoint in checkpoints[1:]:
                ...
                result_file = FLAGS.output_dir+'VIVA-aug-dice-multi-beta-1-alpha-p005-weighted-fusion-'+global_step+'.txt'
                if os.path.exists(result_file):
                    continue
                # if global_step == '12007':
                #     continue
                ...
                # if os.path.exists(result_file):
                #     continue
                # for th in np.arange(0.15, 0.7, 0.05):
                # result_file = FLAGS.output_dir+'VIVA-aug-dice-multi-beta-1-weighted-fusion-'+global_step+'.txt'
                with open(result_file, 'w') as f:
                    ...
                    for im_fn in im_fn_list:
                        ...
                        #updated
                        boxes, timer = detect(im_fn, im, ratio_h, ratio_w, score_map=score[3], geo_map=geometry[3], timer=timer)
            ...
            # print heatmaps[0]
            # StorageDictionary.dict2file_json(FLAGS.heatmap_output_dir+'test_heatmaps_partI.json', heatmaps)
            # with open(FLAGS.heatmap_output_dir+'test_heatmaps.json', 'w') as outfile:
            #     outfile.write(json.dumps(heatmaps))
